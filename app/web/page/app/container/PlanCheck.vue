<template>
  <div>
    ...
    <div class="content">
      <van-cell-group>
        <van-cell title="补货用户" :value="user.username" />
        <van-cell title="补货用户等级" :value="getCurrentLevel" />
...
export default {
  ...
  methods: {
    ...
    agreePlan() {
      const { _id } = this.data;
      if (!_id) return;
      this.$http.post(`/api/plan/agree/${_id}`).then(res => {
        if (res && !res.error) {
          const { id } = this.$route.params;
          this.getData(id);
    ...
    rejectPlan() {
      const { _id } = this.data;
      if (!_id) return;
      this.$http.post(`/api/plan/reject/${_id}`).then(res => {
        if (res && !res.error) {
          const { id } = this.$route.params;
          this.getData(id);
